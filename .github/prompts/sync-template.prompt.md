...
[WHEN TO RUN]
Run this prompt after the `init-project` skill has scaffolded the project and any time after that when:
...
Note: on a freshly cloned, not-yet-initialized template, most of this audit doesn't apply yet — that's expected. Run the `init-project` skill first.
...
2. **Prompts index**: Does `.github/prompts/README.md` list every `.prompt.md` file currently in that folder? Are there prompts listed as *available* that no longer exist? Its "workflows that are skills, not prompts" table deliberately names removed prompt files so a reader can find where each went — that is not drift, and lines marked `inherited-docs-ok` are the ones to leave alone. Extend that table when a workflow moves; do not delete it.
3. **Root README**: Does the `## Project Structure` section in `README.md` match the actual folder layout? Has the one-time "Getting started" section (pointing at the `init-project` skill) been removed now that init is done?
